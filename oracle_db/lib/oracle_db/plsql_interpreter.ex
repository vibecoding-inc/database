defmodule OracleDb.PlsqlInterpreter do
  ...
  """
  @spec execute_function(GenServer.server(), String.t(), list()) :: {:ok, any()} | {:error, String.t()}
  ...
  defp replace_string_literals(body) do
    ...
    {result, literals} = 
      ...
      |> Enum.reduce({body, %{}}, fn {[match], idx}, {text, lits} ->
        ...
        {String.replace(text, match, placeholder, global: false), Map.put(lits, placeholder, match)}
      end)
  ...
  defp parse_if_statement(stmt) do
    ...
    case Regex.run(~r/IF\s+(.+?)\s+THEN\s+(.+?)(?:\s+ELSE\s+(.+?))?\s*END\s+IF/is, stmt) do
      [_, condition, then_block, else_block] ->
        {:if, parse_condition(condition), parse_statements(then_block), parse_statements(else_block)}
  ...
  defp parse_for_statement(stmt) do
    case Regex.run(~r/FOR\s+(\w+)\s+IN\s+(\d+)\s*\.\.\s*(\d+)\s+LOOP\s+(.+?)\s+END\s+LOOP/is, stmt) do
      [_, var, start_val, end_val, body] ->
        {:for, String.upcase(var), String.to_integer(start_val), String.to_integer(end_val), parse_statements(body)}
      nil ->
        # Try parsing FOR with REVERSE
        case Regex.run(~r/FOR\s+(\w+)\s+IN\s+REVERSE\s+(\d+)\s*\.\.\s*(\d+)\s+LOOP\s+(.+?)\s+END\s+LOOP/is, stmt) do
          [_, var, start_val, end_val, body] ->
            {:for_reverse, String.upcase(var), String.to_integer(start_val), String.to_integer(end_val), parse_statements(body)}
          nil ->
            {:unknown, stmt}
        end
    end
  ...
  defp execute_statement({:try, statements, handlers}, context) do
    case execute_statements(statements, context) do
      ...
      {:error, error_name} ->
        ...
        handler = Enum.find(handlers, fn {name, _} -> 
  ...
  defp substitute_variables(sql, context) do
    ...
    result = Regex.replace(~r/:(\w+)/, sql, fn _, var_name ->
  ...
  defp evaluate_expression({:var, var_name}, context) do
    case Map.fetch(context.variables, var_name) do
      {:ok, value} -> {:ok, value}
      :error -> {:ok, nil}  # Undefined variable returns NULL
    end
  end

  defp evaluate_expression({:concat, exprs}, context) do
    results = Enum.map(exprs, fn expr ->
      case evaluate_expression(expr, context) do
        {:ok, nil} -> ""
  ...
  defp evaluate_function("MOD", [num, divisor | _], _context) when is_number(num) and is_number(divisor) do
    rem(num, divisor)
  end
